...
class DataWarmupService {
  ...
  def dev() {
    ...
    Club club = new Club(name: "Squash Antibes Fitness Club", owner: User.findByUsername('jmguilla'))
    ...
    club = new Club(name: "Fitlane Sophia", owner: User.findByUsername('jmguilla'))
    club.addToMembers(User.findByUsername('jmguilla'))
    club.addToAdmins(User.findByUsername('jmguilla'))
    club.save(failOnError: true, flush: true)
    club = new Club(name: "Fitlane Cannes", owner: User.findByUsername('jmguilla'))
    club.addToMembers(User.findByUsername('jmguilla'))
    club.addToAdmins(User.findByUsername('jmguilla'))
    club.save(failOnError: true, flush: true)
    assert Club.count() == 3
